...
export function register(context: vscode.ExtensionContext)
{
    context.subscriptions.push(vscode.languages.registerCompletionItemProvider('gdscript', {
        provideCompletionItems(document: vscode.TextDocument, position: vscode.Position, token: vscode.CancellationToken, context: vscode.CompletionContext)
        {
            ...
            {
                for(let nameE in attCoeData)
                {
                    var CI = new vscode.CompletionItem(nameE, vscode.CompletionItemKind.Field)
                    CI.detail = attCoeData[nameE]
                    list.push(CI)
                }
...
export const signalData: strArr = {
...
export const attCoeData: strArr = {
    "hp":"生命",
    "cri":"暴击率",
    "maxHp":"最大生命",
    "suck":"吸血",
...
function ld(source: string, target: string)
{
    ...
    for(var k = 0; k <= tl; k++) matrix[0][k] = k
    var cost: number = 0
    for(var i = 1; i <= sl; i++)
    {
        for(var j = 1; j <= tl; j++)
...
function GetStringFromDic(key: string)
{
    var value = (attCoeData[key] + charaData[key]).replace(/undefined/g, "")
    return value
}